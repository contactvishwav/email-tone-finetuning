...
# MPS has a hard 4GB per-NDArray limit that multinomial sampling (do_sample=True)
# exceeds with Gemma's 256k-token vocabulary. CPU has no such constraint.
print("Moving model to CPU for final inference (avoids MPS 4GB NDArray limit)...")
model.to("cpu").float()  # float32: CPU has no native float16 math units; float16 causes NaN in sampling
